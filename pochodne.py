...
def parse_tree(function):
  ...
  for token in tokens:
    ...
    elif token not in ['+', '-', '*', '/', '**', ')', 'sin(', 'cos(', 'log(', 'exp(']:
...
def derivative(parse_tree, variable='x'):
  ...
  if left or right:
    ...
    if operator == '+':
      ...
      return derivative(left, variable)*evaluate(right) + evaluate(left)*derivative(right, variable)

    elif operator == '/':
      return ((derivative(left, variable)*evaluate(right) -
               evaluate(left)*derivative(right, variable))/evaluate(right)**2)
      ...
      return (derivative(right, variable)*sympy.Symbol('e')**evaluate(right))
